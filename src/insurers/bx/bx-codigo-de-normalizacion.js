 ...
 *
 * Harmonizes BX records to the shared homologation contract.
 * - Infers transmissions (AUTO/MANUAL) and strips every alias from version text.
 * - Normalizes drivetrain, engine specs, comfort tokens (COMFORT/CONFORT, etc.).
 * - Preserves trim identifiers while removing numeric doors/occupant noise.
 */
...
/**
 * Fix: BMW/MINI brand separation (BX-specific)
 * BX stores MINI vehicles under BMW brand with modelo="MINI CONVERTIBLE", "MINI COOPER", etc.
 * This function separates them to marca="MINI", modelo="CONVERTIBLE", "COOPER", etc.
 */
function fixBXMINIBrand(marca, modelo) {
 ...
 */
function fixBXTrimSpacing(marca, modelo) {
...
/**
 * Fix: Move SDRIVE/XDRIVE from modelo to version (BX-specific)
 */
function cleanBXDriveSuffix(marca, modelo, version) {
...
const PROTECTED_HYPHEN_TOKENS = [
  ...
  },
  {
    ...
    placeholder: "__BX_PROTECTED_R_LINE__",
    canonical: "R-LINE",
  },
  {
    regex: /\bX[\s-]?DRIVE\b/gi,
    placeholder: "__BX_PROTECTED_X_DRIVE__",
    canonical: "X-DRIVE",
  },
];
...
BX_NORMALIZATION_DICTIONARY.transmission_tokens = Array.from(
  new Set(
...
BX_NORMALIZATION_DICTIONARY.irrelevant_comfort_audio = Array.from(
  new Set(
    BX_NORMALIZATION_DICTIONARY.irrelevant_comfort_audio.map((token) =>
      token
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
        .toUpperCase()
...
// ═══════════════════════════════════════════════════════════════════════════
// PROTECTED SPACE-SEPARATED TRIMS (BX - High STRONIC Volume)
// ═══════════════════════════════════════════════════════════════════════════
// BX has highest STRONIC count (581 occurrences) and diverse trim representation
const PROTECTED_SPACED_TRIMS_BX = [
  // M-series (universal)
  "M SPORT",
  ...
  "GRAND TOURING PLUS",
  "GRAND TOURING I", // BX-specific - 9 cases
  "GRAND TOURING S", // BX-specific - 12 cases (also in El Potosí)
...
function protectTrims(version) {
  ...
  PROTECTED_SPACED_TRIMS_BX.forEach((trim) => {
...
function normalizeCylinders(value = "") {
  ...
  output = output.replace(/\b(\d+)V\b/g, "$1CIL");
  return output;
}
...
function normalizeStandaloneLiters(value = "") {
  ...
  return collapsed.replace(
    /\b(\d+\.\d+)(?=\s|$)(?!\s*(?:L\b|TON|TONELADAS|KG|KILOGRAMOS|PUERTAS|PTS?|OCUP|PASAJEROS?|PAS|HP\b|CIL|SERIE))/gi,
    (match, _raw, offset, source) => {
      ...
      const before = source
        ...
        .toUpperCase();
      if (/(TON|TONELADAS|KG|KILOGRAMOS|PESO|CAB|CHASIS)/.test(before)) {
        return match;
      }
      ...
      const after = source
        ...
        .toUpperCase();
      if (/(PUERTAS|PTS?|OCUP|PASAJEROS?|PAS|CIL|SERIE)/.test(after)) {
        return match;
      }
...
function cleanVersionString(versionString = "", model = "", brand = "") {
  ...
  cleaned = stripTokens(
    cleaned,
    BX_NORMALIZATION_DICTIONARY.transmission_tokens
  );
  cleaned = stripTokens(
    cleaned,
    BX_NORMALIZATION_DICTIONARY.irrelevant_comfort_audio
  );

  // NEW: Fix invalid door counts before final cleanup
  cleaned = fixInvalidDoorCounts(cleaned);

  cleaned = cleaned
    .replace(/\b0\s*(P(?:TAS?|TS?|TA)?|PUERTAS?|PTS)\b/g, " ")
    .replace(/\b0(P(?:TAS?|TS?|TA)?|PUERTAS?|PTS)\b/g, " ")
    .replace(/\b(\d{2,})\s*(P(?:TAS?|TS?|TA)?|PUERTAS?|PTS)\b/g, "$1")
    .replace(/\b(\d{2,})(P(?:TAS?|TS?|TA)?|PUERTAS?|PTS)\b/g, "$1");
...
function normalizeModelo(marca, modelo) {
  ...
  // Issue #2: MAZDA brand prefix removal & hyphenation (BX has "MAZDA CX-5")
  if (marcaUpper === "MAZDA") {
    // Remove "MAZDA " prefix
  ...
  // 4. BMW-specific: SERIE prefix extraction
  ...
  // BX-specific (Req 7.5): Remove brand name from modelo field
  // This is more aggressive than the existing pattern - removes brand from anywhere
  if (marcaUpper) {
    ...
    // Also remove brand variants (first word of brand)
    const brandFirstWord = marcaUpper.split(" ")[0];
    if (brandFirstWord && brandFirstWord.length > 2) {
      const brandFirstWordPattern = new RegExp(
        `^${brandFirstWord.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s+`,
        "gi"
      );
      normalized = normalized.replace(brandFirstWordPattern, "");
    }
  }
...
function cleanBxModel(rawModel = "", marca = "") {
  ...
  if (normalizedMarca === "VOLVO") {
    cleaned = cleaned
      .replace(/\bXC\s+(\d+)\b/g, "XC$1")
      .replace(/\bV\s+(\d+)\b/g, "V$1")
      .replace(/\bS\s+(\d+)\b/g, "S$1")
...
function processBxRecord(record) {
  ...
  // BX-SPECIFIC FIXES - Apply BEFORE other normalization
  // Fix 1: BMW/MINI brand separation
  const brandFix = fixBXMINIBrand(record.marca || "", record.modelo || "");
  record.marca = brandFix.marca;
  record.modelo = brandFix.modelo;

  // Fix 2: SDRIVE/XDRIVE removal from modelo to version
  const driveFix = cleanBXDriveSuffix(
    record.marca || "",
    record.modelo || "",
    versionOriginal
  );
  ...
  record.modelo = fixBXTrimSpacing(record.marca, record.modelo);
  ...
  const modeloLimpio = cleanBxModel(record.modelo || "", record.marca || "");

  const marcaConsolidated = consolidateBrand(record.marca);
  if (marcaConsolidated === "INVALID_BRAND") {
    throw new Error("INVALID_BRAND: Brand is not valid");
  }

  const normalized = {
    origen_aseguradora: "BX",
    id_original: record.id_original,
    marca: normalizeText(marcaConsolidated),
    modelo: normalizeModelo(marcaConsolidated, modeloLimpio || record.modelo),
    anio: record.anio,
    transmision: record.transmision,
